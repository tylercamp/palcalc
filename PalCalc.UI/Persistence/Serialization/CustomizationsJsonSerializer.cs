...
namespace PalCalc.UI.Persistence.Serialization
{
    ...
    internal static class CustomizationsJsonSerializer
    {
        ...
        internal static PalInstanceSnapshotDto ToDto(PalInstance value) => new()
        {
            ...
            Gender = value.Gender,
            PassiveSkills = (value.PassiveSkills ?? []).SkipNull().Select(passive => passive.InternalName).ToList(),
            ActiveSkills = (value.ActiveSkills ?? []).SkipNull().Select(skill => skill.InternalName).ToList(),
            EquippedActiveSkills = (value.EquippedActiveSkills ?? []).SkipNull().Select(skill => skill.InternalName).ToList(),
