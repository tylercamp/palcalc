...
namespace PalCalc.Solver.PalReference
{
    ...
    public class SurgeryTablePalReference : IPalReference
    {
        ...
        public SurgeryTablePalReference(IPalReference input, List<ISurgeryOperation> rawOperations)
        {
            ...
            var effectiveList = EffectivePassives.ModelObjects.ToList();
            var actualList = ActualPassives.ModelObjects.ToList();
            ...
            foreach (var op in Operations)
            {
                ...
                switch (op)
                {
                    case AddPassiveSurgeryOperation apso:
                        ...
                        EffectivePassives = EffectivePassives.Concat(new FPassive(PalDB.SharedInstance, apso.AddedPassive));
                        ActualPassives = ActualPassives.Concat(new FPassive(PalDB.SharedInstance, apso.AddedPassive));
                        break;

                    case ReplacePassiveSurgeryOperation rpso:
                        int removedEffectiveIdx = rpso.RemovedPassive is RandomPassiveSkill ? effectiveList.FindIndex(p => p is RandomPassiveSkill) : effectiveList.IndexOf(rpso.RemovedPassive);
                        // TODO - shouldn't the `else` case be `actualList`?
                        int removedActualIdx = rpso.RemovedPassive is RandomPassiveSkill ? actualList.FindIndex(p => p is RandomPassiveSkill || !effectiveList.Contains(p)) : effectiveList.IndexOf(rpso.RemovedPassive);
                        ...
                        effectiveList.RemoveAt(removedEffectiveIdx);
                        actualList.RemoveAt(removedActualIdx);

                        effectiveList.Add(rpso.AddedPassive);
                        actualList.Add(rpso.AddedPassive);
                        break;
                }
            }
            ...
            EffectivePassives = FPassiveSet.FromModel(PalDB.SharedInstance, effectiveList);
            ActualPassives = FPassiveSet.FromModel(PalDB.SharedInstance, actualList);
