...
namespace PalCalc.UI2.Model.Storage
{
    internal abstract class ICalcStorage
    {
        ...
        protected string PathOf(string path) => Path.Join(BasePath, path);
        ...
        public ICalcStorage(string basePath)
        {
            BasePath = basePath;
        }
    }
    ...
    {
        private static ILogger logger = Log.ForContext(typeof(StorageManager));

        private static string CachePath => "cache";
        private static string SaveCachePath => $"{CachePath}/saves";
        private static string DataPath => "data";


        // path for cached copy of save file data
        private static string SaveCachePathFor(ISaveGame forSaveFile)
        {
            Init();
        ...
        private static string SaveFileDataPath(ISaveGame forSaveFile)
        {
            Init();
        ...
        private static string GameSettingsPath(ISaveGame forSaveFile)
        {
            Init();
