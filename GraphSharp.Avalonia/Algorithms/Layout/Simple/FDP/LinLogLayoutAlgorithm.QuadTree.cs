		...
		class QuadTree
		{
			...
			protected void AddNode2( int nodeIndex, Point nodePos, double nodeWeight, int depth )
			{
				...
				if ( children[childIndex] == null )
				{
					...
					if ( nodePos.X <= middleX )
					{
						newMin = new Point(minPos.X, newMin.Y);
						newMax = new Point(middleX, newMax.Y);
					}
					else
					{
						newMin = new Point(middleX, newMin.Y);
						newMax = new Point(maxPos.X, newMax.Y);
					}
					if ( nodePos.Y <= middleY )
					{
						newMin = new Point(newMin.X, minPos.Y);
						newMax = new Point(newMax.X, middleY);
					}
					else
					{
						newMin = new Point(newMin.X, middleY);
						newMax = new Point(newMax.X, maxPos.Y);
					}
