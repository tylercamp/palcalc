		...
		where TGraph : IBidirectionalGraph<TVertex, TEdge>
	{
		...
		*/
		...
		private double GetGravitationEnergy( int index )
		{
			...
			double dist = new Vector( v.Position.X - baryCenter.X, v.Position.Y - baryCenter.Y ).Length;
		...
		private double GetAttractionEnergy( int index )
		{
			...
			foreach ( var e in v.Attractions )
			{
				...
				double dist = new Vector( e.Target.Position.X - v.Position.X, e.Target.Position.Y - v.Position.Y ).Length;
		...
		private double GetRepulsionEnergy( int index, QuadTree tree )
		{
			...
			double dist = new Vector( v.Position.X - tree.Position.X, v.Position.Y - tree.Position.Y ).Length;
		...
		private void ComputeBaryCenter()
		{
			...
			foreach ( var v in vertices )
			{
				...
				baryCenter = new Point(
					v.Position.X * v.RepulsionWeight,
					v.Position.Y * v.RepulsionWeight
				);
			}
			if ( repWeightSum > 0.0 )
			{
